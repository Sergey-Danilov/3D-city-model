...
double x, y, z;//ѕоложение камеры в пространстве
...
// «агрузка картинки и конвертирование в текстуру
int LoadGLTextures()
{
	int Status=FALSE;  
	// «агрузка картинки
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	for (int i = 0; i < 6; i++)//создаем 6 р€дов домов
	{
		mapp.CreateHouse(4, 5 - 5 * i, 3, 3, 5);//4-к. по горизонтали, 5-5*i - к. углублени€, 3 - длина, 3 - ширина, 5 - этажи
					...
					int			nCmdShow)			// Window Show State
{
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			if ((keys[0x57])||(keys[VK_SPACE]))//W или space - поднимаемс€
			{
				height1 += 0.01;
			}
			if (keys[0x53])//S - опускаемс€
			{
				height1 -= 0.01;
			}
			//дл€ мыши
			GetCursorPos(&mousexy);
			angleX +=(320 - mousexy.x)/2; //2 Ч чувствительность 
			angleY +=(240 - mousexy.y)/2;
			SetCursorPos(320,240); // ƒефолтное положение мыши
